 ...
 *
 */
public class CustomWorkHandler {
	
	public static void main(String[] args) {
		
		EnvironmentManager simpleEnv = EnvironmentManager.get();
		RuntimeManager manager = simpleEnv.start("com/example/bpms/camelsupplyitemapproval.bpmn2");
		
		
		try{
			new CustomWorkHandler().startSimulation(manager, simpleEnv.getCamelContext());
		}finally{
			simpleEnv.stop();
		}
